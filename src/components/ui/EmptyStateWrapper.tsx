...
const defaultIconPaths = {
  ...
  compass:
    "M12 0a12 12 0 1 0 0 24 12 12 0 0 0 0-24zM4 12a8 8 0 1 1 16 0 8 8 0 0 1-16 0zM14 12a2 2 0 1 1-4 0 2 2 0 0 1 4 0z", // Basic compass path
...
const Particle: React.FC<ParticleProps> = ({ initialX, initialY, iconPath }) => {
  return (
    <div className="absolute" style={{ left: `${initialX}%`, top: `${initialY}%` }}>
      <svg
        width="24"
        height="24"
        viewBox="0 0 24 24"
...
/** Forma exacta de lo que genera el useEffect de más abajo. */
type ParticleData = {
  id: number;
  initialX: number;
  initialY: number;
  ...
  useEffect(() => {
    if (isEmpty) {
      const generatedParticles = Array.from({ length: particleCount }).map((_, i) => ({
        ...
        iconPath:
          defaultIconPaths[particles[i % particles.length] as keyof typeof defaultIconPaths],
      }));
    ...
    >
      {isEmpty && (
        <div className="absolute inset-0 pointer-events-none z-0">
          {" "}
          {/* Render particles behind content */}
          {particleData.map((p) => (
            <Particle
              key={p.id}
              initialX={p.initialX}
      ...
      <div className="relative z-10">
        {" "}
        {/* Ensure children are above particles */}
        {children}
      </div>
    </div>
  );
