...
// Marcadores de muestra para superficies de marketing (landing). El dashboard
// pasa siempre los viajes reales del usuario (o [] mientras cargan).
const defaultMarkers: PolaroidMarker[] = [
  {
    id: "sf",
    location: [37.78, -122.44],
    image:
      "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=400&h=280&fit=crop&auto=format&q=75",
  ...
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?w=400&h=280&fit=crop&auto=format&q=75",
    caption: "New York",
    rotate: 4,
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=400&h=280&fit=crop&auto=format&q=75",
    caption: "Tokyo",
    rotate: -3,
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?w=400&h=280&fit=crop&auto=format&q=75",
    caption: "Sydney",
    rotate: 6,
  },
  {
    ...
    location: [48.86, 2.35],
    image:
      "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?w=400&h=280&fit=crop&auto=format&q=75",
    caption: "Paris",
    rotate: -4,
  },
  {
    ...
    location: [51.51, -0.13],
    image:
      "https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?w=400&h=280&fit=crop&auto=format&q=75",
    caption: "London",
    rotate: 3,
  },
];
...
export function GlobePolaroids({
  ...
  // Los marcadores cambian sin recrear el globo: cobe soporta update({ markers }).
  // Antes cada geocodificación resuelta destruía y reconstruía el WebGL entero.
  useEffect(() => {
  ...
  const handlePointerUp = useCallback((e: PointerEvent) => {
    ...
    if (pointerInteracting.current !== null) {
      ...
      thetaOffsetRef.current = Math.max(
        -0.5,
        Math.min(0.5, thetaOffsetRef.current + dragOffset.current.theta),
      );
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      globeRef.current = createGlobe(canvas!, {
        ...
        mapBrightness: 9,
        baseColor: [1, 1, 1],
        markerColor: [0.12, 0.42, 0.6],
        glowColor: [0.94, 0.93, 0.91],
        markers: markersRef.current.map((m) => ({ location: m.location, size: 0.06 })),
      });
  ...
  return (
    ...
    >
      {/* Halo de aparición: un brillo cálido que respira una vez cuando el
          globo termina de cargar y se asienta como resplandor sutil. */}
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-4 rounded-full"
        style={{
          background:
            "radial-gradient(circle, rgba(125,211,252,0.35) 0%, rgba(56,189,248,0.12) 45%, transparent 70%)",
          opacity: ready ? 1 : 0,
          transform: ready ? "scale(1)" : "scale(0.85)",
          transition:
            "opacity 1.4s cubic-bezier(0.23, 1, 0.32, 1), transform 1.4s cubic-bezier(0.23, 1, 0.32, 1)",
          animation: ready
            ? "globe-glow-settle 2.8s cubic-bezier(0.23, 1, 0.32, 1) forwards"
            : undefined,
        }}
      />
      <canvas
      ...
      {popup && (
        ...
        >
          <div
            className="w-48 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/8"
            style={{ animation: "globe-popup-in 200ms cubic-bezier(0.23, 1, 0.32, 1) both" }}
          >
            {popup.marker.image && (
