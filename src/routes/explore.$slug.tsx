...
export const Route = createFileRoute("/explore/$slug")({
  ...
  pendingComponent: PublicTripSkeleton,
...
// Mostrado por el router mientras el loader de la ruta resuelve (navegación
// cliente a un itinerario público) — misma forma que la página real.
function PublicTripSkeleton() {
  return (
    <div className="min-h-dvh animate-pulse bg-gradient-to-br from-[#D6EAF8] via-white to-[#B8D4E8]">
      <div className="sticky top-0 z-20 border-b border-sky-100/60 bg-white/80 px-4 py-3">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <div className="h-7 w-28 rounded bg-sky-100" />
          <div className="h-9 w-24 rounded-full bg-sky-100" />
        </div>
      </div>
      <div className="h-72 w-full bg-sky-100 md:h-96" />
      <div className="mx-auto max-w-5xl px-4 py-8 md:py-10">
        ...
        <div className="space-y-6">
          {[0, 1].map((day) => (
            <div key={day} className="overflow-hidden rounded-3xl bg-white/70">
              <div className="h-48 w-full bg-sky-100 md:h-64" />
              <div className="space-y-3 p-5 md:p-6">
                {[0, 1, 2].map((act) => (
                  <div key={act} className="h-16 rounded-2xl bg-sky-50" />
                ))}
              </div>
            </div>
          ))}
