...
// Uses Resend's structured error `.status` when available, falls back to
...
async function moveToDlq(
  // El cliente se usa solo para pgmq (RPC), que no está en el tipo Database
  // generado; tiparlo con Database haría fallar cada llamada.
...
export const Route = createFileRoute("/email/email/queue/process")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        for (const queue of ["auth_emails", "transactional_emails"]) {
          ...
          const messageIds = Array.from(
            new Set(
              messages
                // Filas devueltas por pgmq.read, fuera del tipo generado.
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                .map((msg: any) =>
