...
function WelcomePage() {
  ...
  const trialPatch = () =>
    hadTrial ? {} : { trial_ends_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() };
  ...
  return (
    <div className="relative min-h-dvh overflow-hidden bg-gradient-to-br from-[#D6EAF8] via-white to-[#B8D4E8]">
      ...
      <div className="relative mx-auto flex min-h-dvh max-w-xl flex-col items-center justify-center px-6 py-12">
        ...
        {/* Una sola pantalla: edad e idioma. Ambos afinan el itinerario y
            ninguno merece un paso propio delante del producto. */}
        <div className="w-full overflow-hidden rounded-3xl bg-white/85 p-8 shadow-[0_20px_60px_-15px_rgba(46,107,138,0.25)] ring-1 ring-white/60 backdrop-blur-xl">
          ...
          <div className="mt-6 text-center">
            <div className="font-display text-6xl font-bold text-[#1E6B9A]">{ageLabel}</div>
          ...
          <p className="mt-8 text-sm font-semibold text-sky-900">{t("welcome.langTitle")}</p>
            ...
            {LANGUAGE_OPTIONS.map((l) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-3 rounded-2xl border-2 px-4 py-3.5 text-left transition-all ${
                    selected
          ...
          <button
            onClick={finish}
            disabled={saving}
            className="mt-8 flex w-full items-center justify-center gap-2 rounded-full bg-[#1E6B9A] px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-[#1E6B9A]/25 transition hover:bg-[#15577E] disabled:opacity-60"
          >
